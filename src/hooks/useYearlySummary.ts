...
import { generateYearlySummariesForPastYears } from '@/lib/yearly-summary-utils'
...
export function useYearlySummary() {
  ...
  const generateYearlySummaries = async (): Promise<{ success: boolean; count: number }> => {
  ...
  // Auto-generate yearly summaries for all past years that need them when dashboard is visited
  useEffect(() => {
    ...
    const checkAndGenerate = async () => {
      console.log('Auto-generating yearly summaries...')
      try {
        const result = await generateYearlySummaries()
        if (result.success) {
          setLastGenerationCheck(now)
          console.log('Yearly summary generation completed:', result)
        }
      } catch (error) {
        console.error('Auto yearly summary generation failed:', error)
      }
    }
