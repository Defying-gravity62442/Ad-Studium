...
export function CalendarSync({ milestones, onClose }: CalendarSyncProps) {
  ...
  // Memoize incomplete milestones to prevent unnecessary recalculations
  ...
  // Auto-select incomplete milestones only once when component mounts
  useEffect(() => {
  ...
  const handleSync = useCallback(async () => {
    ...
    setShowResults(true)
  }, [selectedMilestones, milestones, syncMilestones])
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {milestone.dueDate && (
                    <p className="text-xs text-gray-500 mt-1">
                      Due: {(() => {
                        const [year, month, day] = milestone.dueDate.split('T')[0].split('-')
